...
from app.models import Event, User, TicketType, Booking
from app import db
import json
import qrcode
import io
...
@bp.route("/api/ticket-info/<int:event_id>")
def get_ticket_info(event_id):
    ...
    for ticket in ticket_types:
        ticket_info[ticket.name] = ticket.stock
...
@bp.route("/confirm-seats", methods=['POST'])
def confirm_seats():
            ...
            if quantity > 0:
                ...
                if ticket_type:
                    ...
                    booking = Booking(
                        ...
                        quantity=quantity,
                        selected_seats=json.dumps(seats_for_ticket),
...
@bp.route('/api/booked-seats/<int:event_id>')
